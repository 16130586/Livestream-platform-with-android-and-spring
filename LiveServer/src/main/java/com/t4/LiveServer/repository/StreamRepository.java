...
public interface StreamRepository extends JpaRepository<Stream, Integer> {
    ...
    List<Stream> findByStreamNameContaining(String streamName, Pageable pageable);
    @Query(nativeQuery = true,
            value = "SELECT s.* FROM stream as s inner join stream_type as st " +
                    "on s.stream_id = st.stream_id inner join types as t on t.type_id = st.type_id " +
                    "where t.type_name in :streamTypes and s.stream_name like %:streamName% group by s.stream_id having count(s.stream_id) = :countType")
